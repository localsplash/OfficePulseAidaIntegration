...
3. Add a Realtime switch only to each reviewed tenant context and each dedicated managed inbound context. The normal default family form is `switch => Realtime`; its lookup context must match `didContext`. Do not install a catch-all Realtime switch in a trunk, shared or default context. Preserve any reviewed tenant outbound/static routing.
4. In the trunk ingress, replace the single reviewed static DID entry with an explicit `Goto(managed-concierge,+19496501147,1)`, or an equivalent reviewed delegation. Remove the old winning queue/provider route for that exact DID. A matching static route can shadow Realtime even when SQL writes succeed. No automatic adoption or static-file editing occurs in the API.
