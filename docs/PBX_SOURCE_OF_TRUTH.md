...
```json
{"source":"asterisk","iTenantId":1,"provisioningEnabled":false,"contexts":["business-one"],"extensions":[{"id":"sip-101","extension":null,"context":"business-one","callerId":"Alice","transport":"transport-udp","aors":"aor-101","applyState":"unknown"}]}
```
...
```json
{"source":"asterisk","iTenantId":1,"provisioningEnabled":false,"queues":[{"id":"support-one","name":"support-one","strategy":"rrmemory","applyState":"unknown","members":[{"interface":"Local/101@business-one","memberName":"Alice","penalty":0,"paused":false}]}]}
```
...
Inventory does not require LiveKit or Agent setup. The canonical development service uses `VOICE_ENABLED=false`; inventory is independent of voice connectors. The separate account needs SELECT on `ps_endpoints`, `queues`, and `queue_members`, plus `extensions` for exact managed queue ownership markers, including when writes are disabled; see `deploy/sql/pbx-inventory-grants.sql`. Inventory itself never writes. The separately enabled POC writer follows [PBX_PROVISIONING.md](PBX_PROVISIONING.md). Startup never creates queue tables or changes Asterisk mappings.
