...
export async function migrateRuntime(config: RuntimeMysqlConfig): Promise<void> {
  ...
  try {
    ...
    for (const name of ['runtime-schema.sql', '002_device_access.sql', '003_event_receipts.sql', '004_remove_retired_pbx.sql', '005_agent_admission.sql', '006_call_scope.sql', '007_handset_devices.sql']) {
