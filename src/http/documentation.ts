import { handsetPaths, handsetSchemas } from '../devices/openApi.js';
...
export const openApi = {
  ...
  paths: {
    ...agentPaths,
    ...handsetPaths,
    '/healthz': { get: { tags: ['Health'], summary: 'Process health', responses: { '200': response('Process is responding', { type: 'object', properties: { status: { type: 'string', example: 'ok' }, pbxInstanceId: { type: 'string' }, environmentName: { type: 'string', enum: ['dev','staging','prod'] }, version: { type: 'string', example: '2026.9.14.14.30', description: 'Pacific (America/Los_Angeles, PST/PDT) commit timestamp (YYYY.M.D.H.M); -dirty for uncommitted changes; unbuilt in source development mode' }, revision: { type: 'string', nullable: true, description: 'Full Git commit ID' }, sourceUpdatedAt: { type: 'string', format: 'date-time', nullable: true, description: 'Commit time with Pacific UTC offset' }, timeZone: { type: 'string', example: 'America/Los_Angeles' }, dirty: { type: 'boolean', nullable: true } } }) } } },
  ...
  components: { securitySchemes: { handsetBearer: { type: 'http', scheme: 'bearer' } }, schemas: {
    ...handsetSchemas,
    Error: { type: 'object', properties: { error: { type: 'string' }, message: { type: 'string' }, details: { type: 'array', items: { type: 'string' } } } },
    Readiness: { type: 'object', properties: { environmentName: { type: 'string', enum: ['dev','staging','prod'] }, pbxInstanceId: { type: 'string', pattern: '^[A-Za-z0-9_.-]{1,80}$' }, ready: { type: 'boolean' }, fullyOperational: { type: 'boolean' }, components: { type: 'object', additionalProperties: { type: 'object', properties: { ready: { type: 'boolean' }, criticality: { type: 'string', enum: ['critical', 'degraded'] }, detail: { type: 'string' }, since: { type: 'string', format: 'date-time' } } } } } },
    ...pbxSchemas,
