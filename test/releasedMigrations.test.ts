...
test('released migration ledger remains compatible before applying projection cleanup', async (t) => {
  ...
  assert.deepEqual(checked, [...Object.keys(RELEASED_CHECKSUMS), '004_remove_retired_pbx.sql', '005_agent_admission.sql', '006_call_scope.sql', '007_handset_devices.sql']);
  assert.deepEqual(applied, ['004_remove_retired_pbx.sql', '005_agent_admission.sql', '006_call_scope.sql', '007_handset_devices.sql']);
});
